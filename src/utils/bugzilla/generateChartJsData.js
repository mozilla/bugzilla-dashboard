...
import toMondayOfWeek from '../toMondayOfWeek';
...
const bugsGroupedByWeek = (bugs, startDate) => (
  bugs.reduce((result, { creation_time, cf_last_resolved }) => {
    ...
    const createdDate = toMondayOfWeek(creation_time, startDate);
    ...
    if (cf_last_resolved) {
      const resolvedDate = toMondayOfWeek(cf_last_resolved, startDate);
