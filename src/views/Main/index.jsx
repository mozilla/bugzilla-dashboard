...
class MainContainer extends Component {
    ...
    getReformattedData = (data) => {
      ...
      Object.values(data).forEach((product) => {
        returnData.push({
          ...product[0],
          product: '',
          metrics: product[0].metrics,
          componentData: Object.values(product[0].componentData).map(
            component => ({ ...component, metrics: component.metrics }),
          ),
        });
      });
      return returnData;
    ...
    getBugsCountHandler = (bugzillaComponents, type) => {
      Object.values(bugzillaComponents).map(async (componentData) => {
        const { product, component } = componentData;

        const { metrics } = type === 'parent'
          ? bugzillaComponents[`${product}`]
          : bugzillaComponents[`${product}::${component}`];
        await Promise.all(
        ...
        if (componentData.componentData) {
          this.getBugsCountHandler(componentData.componentData, 'child');
        }
  ...
  getProductsAndMetrics = (componentsOwned, mail) => {
    const result = chain(componentsOwned)
      .groupBy('product')
      .each(item => item)
      .value();
    const returnData = Object.values(result).reduce((acc, product, i) => {
      const componentsList = product.map(pro => pro.component);
      const componentData = componentsList.reduce(
        (childComponents, component) => {
          const data = childComponents;
          data[`${Object.keys(result)[i]}::${component}`] = {
            label: `${Object.keys(result)[i]}::${component}`,
            bugzillaEmail: mail,
            product: [Object.keys(result)[i]],
            component: [component],
            metrics: {},
          };
          return data;
  ...
  async bugzillaComponents(bzOwners, partialOrg) {
    ...
    let concatination = {};
    const bugzillaComponents = Object.values(partialOrg).reduce(
      (result, { bugzillaEmail, mail }) => {
        const componentsOwned = bzOwners[bugzillaEmail] || bzOwners[mail];
        if (componentsOwned) {
          concatination = {
            ...concatination,
            ...this.getProductsAndMetrics(
              componentsOwned,
              bugzillaEmail || mail,
            ),
          };
        }
        return { ...result, ...concatination };
      },
      {},
    );
    ...
    this.getBugsCountHandler(bugzillaComponents, 'parent');
  }
  ...
  render() {
    ...
    const result = chain(bugzillaComponents)
      .groupBy('product')
      .value();
          ...
          <Suspense fallback={<div>Loading...</div>}>
            <Switch>
              ...
              <PropsRoute
                ...
                bugzillaComponents={this.getReformattedData(result)}
